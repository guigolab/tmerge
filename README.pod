...
<p>blahblah<img src="http://public-docs.crg.es/rguigo/CLS/img/tmerge_1.png" alt="tmerge sketch" /></p>

=end HTML

B<Compatibility> between aligned structures is evaluated using the following rules:

=over

=item A pair of B<spliced reads> is deemed compatible if:

=over

=item (1) it shows exonic overlap on the same genomic strand,

and

=item (2) either their intron chains are equal, or one is a subset of the other

=back

Condition (2) means that C<tmerge>, contrary to other transcript mergers, will never artificially chain exons in the output:

=begin HTML

<p><img src="https://public-docs.crg.es/rguigo/CLS/img/tmerge_non_merge.png" alt="tmerge sketch" /></p>
