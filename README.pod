...
=item * C<exonIntronOverhangMerge> (integer) = maximum number of nucleotides of exon/intron overhang at a splice site for two transcripts to be considered compatible (i.e., mergeable):

=begin HTML

<p><img src="http://public-docs.crg.es/rguigo/CLS/img/tmerge_exonIntronOverhangMerge.png" alt="tmerge exonIntronOverhangMerge sketch" /></p>

=end HTML


This applies only to terminal exons of a shorter transcript that overlap an intron of a longer one. Setting this option to a positive value can correct mismapped splice junctions that sometimes occur when aligning very short terminal read exons.
