...
C<tmerge> outputs non-redudant transcript models (B<TMs>) in GTF format. Each TM entry is uniquely identified by its C<transcript_id> attribute.
...
=item UUID::Generator::PurePerl (CPAN)
Install (bash): C<< $ cpanm UUID::Generator::PurePerl >>

=item Parallel::ForkManager (CPAN)
